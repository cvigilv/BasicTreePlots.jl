...
for (index, layoutstyle) in enumerate(layoutstyles)
    ...
    ax = Axis(fig[row, column]; title = string(layoutstyle), aspect = DataAspect())
    hidedecorations!(ax)
    hidespines!(ax)
    tp = treeplot!(tree; layoutstyle)
    for clade_index in reverse(eachindex(nested_clades))
        treehilight!(
            tp;
            nodes = [nested_clades[clade_index]],
            color = nested_colors[clade_index],
            padding = 0.08,
            strokecolor = nested_colors[clade_index],
            strokewidth = 1,
        )
    end
end

save("examples/unrooted_comparison.png", fig)
fig
